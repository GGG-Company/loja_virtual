...
import { RotateCcw, Search, Filter, Clock, CheckCircle, XCircle, Truck, Package, AlertCircle, Eye, ChevronLeft, ChevronRight } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-600 mt-1">Gerencie solicitações de devolução e reembolsos</p>
          </div>
        </div>
      </div>
        ...
        {statusOptions.slice(1, 7).map((opt) => {
          ...
          return (
            <button
              ...
              className={`p-4 rounded-lg border transition-colors ${statusFilter === opt.value ? "border-primary-600 bg-primary-50" : "border-gray-200 hover:bg-gray-50"}`}
            >
      ...
      <div className="bg-white rounded-lg shadow p-4">
        <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar por número, pedido, cliente..." className="pl-10" />
          </div>
          <select
            value={statusFilter}
            onChange={(e) => {
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-lg shadow p-8 text-center">
          ...
          <p className="text-gray-600">{statusFilter !== "all" ? "Tente alterar o filtro de status" : "Ainda não há solicitações de devolução"}</p>
        </div>
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Devolução</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {returns.map((ret) => {
                  ...
                  return (
                    <motion.tr key={ret.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-gray-50">

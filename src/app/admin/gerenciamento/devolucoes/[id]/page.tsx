...
import { ArrowLeft, Package, User, MapPin, Clock, CheckCircle, XCircle, Truck, AlertCircle, FileText, DollarSign, ExternalLink, Printer } from "lucide-react";
...
export default function AdminDevolucaoDetalhePage() {
  ...
  }, [authStatus, returnId, router, loadReturn]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{returnData.returnNumber}</h1>
            <p className="text-gray-600">Solicitado em {new Date(returnData.createdAt).toLocaleString("pt-BR")}</p>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-4">
              {returnData.items.map((item) => {
                const orderItem = returnData.order.items.find((oi) => oi.id === item.orderItemId);
                const product = orderItem?.product;

                return (
                  <div key={item.id} className="flex items-center gap-4 p-3 border rounded-lg">
                    <div className="w-16 h-16 bg-gray-100 rounded flex items-center justify-center overflow-hidden">{product?.imageUrl ? <Image src={product.imageUrl} alt={product.name || ""} width={64} height={64} className="object-cover" /> : <Package className="h-8 w-8 text-gray-400" />}</div>
                    ...
                    <div className="text-right">
                      <p className="font-bold text-green-600">R$ {((orderItem?.price || 0) * item.quantity).toFixed(2)}</p>
                    </div>
                  </div>
                );
              })}
            ...
            <div className="mt-4 p-4 bg-gray-50 rounded-lg">
              ...
              {returnData.reasonDetails && <p className="text-gray-600 mt-2 italic">"{returnData.reasonDetails}"</p>}
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-lg shadow p-6">
            ...
            <div className="mb-4">
              ...
              <Textarea value={adminNotes} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setAdminNotes(e.target.value)} placeholder="Adicione notas sobre esta devolução..." rows={3} />
              <Button size="sm" variant="outline" className="mt-2" onClick={() => performAction("update_notes")} disabled={actionLoading}>
                Salvar Notas
              </Button>
            </div>
            ...
            <div className="border-t pt-4 space-y-4">
              ...
              {["REQUESTED", "PENDING_REVIEW"].includes(returnData.status) && (
                <div className="flex gap-2">
                  <Button onClick={() => performAction("approve")} disabled={actionLoading} className="flex-1">
              ...
              {returnData.status === "APPROVED" && (
                <div>
                  <p className="text-sm text-gray-600 mb-2">O pedido original {returnData.order.melhorEnvioOrderId ? "possui" : "não possui"} etiqueta do Melhor Envio.</p>
                  <Button onClick={() => performAction("generate_label")} disabled={actionLoading || !returnData.order.melhorEnvioOrderId} className="w-full">
                    <FileText className="h-4 w-4 mr-2" />
                    Gerar Etiqueta de Devolução
                  </Button>
                  {!returnData.order.melhorEnvioOrderId && <p className="text-xs text-red-500 mt-1">Não é possível gerar etiqueta reversa pois o pedido original não usou Melhor Envio.</p>}
                </div>
              )}
              ...
              {returnData.status === "IN_TRANSIT" && (
                <Button onClick={() => performAction("mark_received")} disabled={actionLoading} className="w-full">
              ...
              {returnData.status === "RECEIVED" && (
                <Button onClick={() => performAction("start_inspection")} disabled={actionLoading} className="w-full">
              ...
              {returnData.status === "INSPECTING" && (
                <div className="space-y-3">
                  ...
                  <Button onClick={() => performAction("approve_refund", { refundAmount: parseFloat(refundAmount) })} disabled={actionLoading || !refundAmount} className="w-full">
              ...
              {returnData.status === "REFUND_PENDING" && (
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <Input value={refundReference} onChange={(e) => setRefundReference(e.target.value)} placeholder="Ex: PIX123456" />
                  </div>
                  <Button onClick={() => performAction("complete_refund", { refundReference })} disabled={actionLoading} className="w-full bg-green-600 hover:bg-green-700">
                    <CheckCircle className="h-4 w-4 mr-2" />
              ...
              {returnData.status === "REFUNDED" && (
                <div className="p-4 bg-green-50 rounded-lg text-center">
                  ...
                  <p className="text-sm text-green-600">
                    Reembolso de R$ {returnData.refundAmount?.toFixed(2)} realizado em {returnData.refundedAt ? new Date(returnData.refundedAt).toLocaleDateString("pt-BR") : "-"}
                  </p>
                  {returnData.refundReference && <p className="text-xs text-gray-500 mt-1">Ref: {returnData.refundReference}</p>}
                </div>
              )}
            </div>
          </motion.div>
        ...
        <div className="space-y-6">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-2 text-sm">
              ...
              {returnData.user.phone && <p className="text-gray-600">{returnData.user.phone}</p>}
            </div>
          </motion.div>
